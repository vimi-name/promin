# Швидкий старт «якщо — то»

- Якщо папка порожня, але є мета → дайте мету й запустіть `promin init`.
- Якщо є repository → відкрийте його root і запустіть `promin init`.
- Якщо є backlog → покладіть файл у repository або вкажіть його в brief.
- Якщо plan правильний → підтвердьте.
- Якщо щось треба змінити → напишіть уточнення природною мовою.
- Якщо потрібні явні налаштування → відкрийте expert config після preview.
